...
const props = defineProps({
  source: { type: String, default: 'algorithm' },
...
const description = computed(
  () =>
    ...
    (isAi.value
      ...
      : 'Kết quả được hệ thống tính trực tiếp bằng công thức; không gọi mô hình AI.'),
)
</script>
...
.analysis-source-badge--ai {
  color: #5b21b6;
  background: #f1eaff;
  border: 1px solid #d8c4ff;
}
.analysis-source-badge--algorithm {
  color: #1559a7;
  background: #e9f3ff;
  border: 1px solid #c6ddf8;
}
.analysis-source-badge__tooltip {
  position: absolute;
  right: 0;
  top: calc(100% + 9px);
  ...
  line-height: 1.45;
  letter-spacing: 0;
  box-shadow: 0 12px 30px rgba(15, 35, 65, 0.24);
  opacity: 0;
  visibility: hidden;
...
.analysis-source-badge__tooltip::before {
  content: '';
  position: absolute;
  right: 18px;
  top: -5px;
...
.analysis-source-badge:hover .analysis-source-badge__tooltip,
.analysis-source-badge:focus .analysis-source-badge__tooltip,
.analysis-source-badge:focus-within .analysis-source-badge__tooltip {
  opacity: 1;
  visibility: visible;
