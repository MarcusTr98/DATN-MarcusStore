...
public class OrderAssignmentServiceImpl implements OrderAssignmentService {
        ...
        private static final Set<String> ACTIVE_ORDER_STATUSES = Set.of(
                        "PENDING", "CONFIRMED", "PROCESSING", "READY_FOR_PICKUP", "PACKED", "SHIPPING", "DELIVERED",
                        "FAILED");
        ...
        public void assignAutomatically(Order order) {
                ...
                if (selectedStaff != null) {
                        createAssignment(lockedOrder, selectedStaff, null, "AUTO",
                                        "Hệ thống tự phân theo số đơn đang xử lý");
                }
        }
                ...
                User staff = userRepository.findActiveStaffWithOrderUpdatePermissionById(staffId)
                                .orElseThrow(
                                                () -> new IllegalArgumentException(
                                                                "Nhân viên không hoạt động hoặc không có quyền xử lý đơn"));
        ...
        @Override
        @Transactional(readOnly = true)
        public OrderAssignmentDashboardResponse getDashboard(int pendingPage, int pendingSize) {
                ...
                for (User staff : staffs) {
                        activeLoadByStaffId.put(staff.getUserId(),
                                        assignmentRepository.countCurrentActiveOrders(staff.getUserId(),
                ...
                List<OrderAssignmentDashboardResponse.StaffLoad> staffLoads = staffs.stream().map(staff -> {
                        ...
                        long selfAssigned = assignmentRepository.countAssignmentsByTypeSince(staff.getUserId(), "SELF",
                                        kpiSince);
                        long autoAssigned = assignmentRepository.countAssignmentsByTypeSince(staff.getUserId(), "AUTO",
                                        kpiSince);
                        long manualAssigned = assignmentRepository.countAssignmentsByTypeSince(staff.getUserId(),
                                        "MANUAL",
                                        kpiSince);
                        long totalAssigned = selfAssigned + autoAssigned + manualAssigned;
                        long completedInPeriod = assignmentRepository.countCompletedAssignmentsSince(staff.getUserId(),
                                        kpiSince);
                        return OrderAssignmentDashboardResponse.StaffLoad.builder()
                                        .staffId(staff.getUserId()).staffName(displayName(staff))
                                        ...
                                        .periodCompletionRate(totalAssigned == 0 ? 0
                                                        : Math.round(Math.min(completedInPeriod, totalAssigned) * 1000.0
                                                                        / totalAssigned) / 10.0)
                                        .build();
                }).toList();
                Map<Integer, Long> projectedActiveCountByStaffId = new HashMap<>(activeLoadByStaffId);
                Map<Integer, Long> projectedWorkloadByStaffId = staffs.stream().collect(
                                Collectors.toMap(User::getUserId, this::workloadScore));
                Page<Order> pendingOrderPage = orderRepository
                                .findPendingUnassignedOrders(PageRequest.of(pendingPage, pendingSize));
                List<OrderAssignmentDashboardResponse.PendingOrder> pendingOrders = pendingOrderPage.stream()
                                .map(order -> {
                                        User planned = selectLeastLoadedStaff(
                                                        staffs, projectedActiveCountByStaffId,
                                                        projectedWorkloadByStaffId);
                                        if (planned != null) {
                                                projectedActiveCountByStaffId.merge(planned.getUserId(), 1L, Long::sum);
                                                projectedWorkloadByStaffId.merge(
                                                                planned.getUserId(),
                                                                (long) statusWeight(order.getOrderStatus()), Long::sum);
                                        }
                                        return OrderAssignmentDashboardResponse.PendingOrder.builder()
                                                        .orderCode(order.getOrderCode())
                                                        .recipientName(order.getRecipientName())
                                                        .finalAmount(order.getFinalAmount())
                                                        .autoAssignAt(order.getAutoAssignAt())
                                                        .plannedStaffId(planned == null ? null : planned.getUserId())
                                                        .plannedStaffName(planned == null ? null : displayName(planned))
                                                        .build();
                                }).toList();
                return OrderAssignmentDashboardResponse.builder()
                                .staffLoads(staffLoads)
                                .pendingOrders(pendingOrders)
                                .pendingTotalElements(pendingOrderPage.getTotalElements())
                                .pendingTotalPages(pendingOrderPage.getTotalPages())
                                .pendingPage(pendingOrderPage.getNumber())
                                .build();
        }
        ...
        public StaffAssignmentStatusResponse updateStaffSettings(Integer staffId, boolean acceptingOrders,
                ...
                User staff = userRepository.findEligibleStaffByIdForAssignment(staffId)
                                .orElseThrow(() -> new IllegalArgumentException(
                                                "Nhân viên không đủ điều kiện xử lý đơn"));
                staff.setAcceptingOrders(acceptingOrders);
        ...
        private User currentStaff() {
                return userRepository.findActiveStaffWithOrderUpdatePermissionById(SecurityUtils.getCurrentUserId())
                                .orElseThrow(() -> new IllegalStateException(
                                                "Bạn không phải nhân viên xử lý đơn đang hoạt động"));
        }
        ...
        private StaffAssignmentStatusResponse buildStaffStatus(User staff) {
                ...
                boolean cooldownReady = staff.getLastAssignedAt() == null
                                || !staff.getLastAssignedAt().plusSeconds(SELF_CLAIM_COOLDOWN_SECONDS)
        ...
        private User selectLeastLoadedStaff(boolean lockForAssignment) {
                ...
                return staffs.stream()
                                ...
                                .min(Comparator.comparingLong(this::workloadScore)
        ...
        private User selectLeastLoadedStaff(List<User> staffs, Map<Integer, Long> activeCountByStaffId,
                        Map<Integer, Long> workloadByStaffId) {
                return staffs.stream()
                                .filter(staff -> Boolean.TRUE.equals(staff.getAcceptingOrders()))
                                .filter(staff -> activeCountByStaffId.getOrDefault(staff.getUserId(),
                                                0L) < maxActiveOrders(staff))
                                .min(Comparator
                                                .comparingLong((User staff) -> workloadByStaffId
                                                                .getOrDefault(staff.getUserId(), 0L))
                                                .thenComparing(staff -> staff.getLastAssignedAt(),
                                                                Comparator.nullsFirst(Comparator.naturalOrder()))
                                                .thenComparing(User::getUserId))
                                .orElse(null);
        ...
        private String displayName(User user) {
                if (user == null)
                        return null;
                return user.getFullName() == null || user.getFullName().isBlank() ? user.getUsername()
                                : user.getFullName();
        }
        ...
        private int maxActiveOrders(User staff) {
                return staff.getMaxActiveOrders() == null || staff.getMaxActiveOrders() < 1 ? 5
                                : staff.getMaxActiveOrders();
        }
        ...
        private long workloadScore(User staff) {
                return assignmentRepository.findCurrentActiveStatuses(staff.getUserId(), ACTIVE_ORDER_STATUSES).stream()
                                .mapToLong(this::statusWeight).sum();
        }
        ...
        private int statusWeight(String status) {
                return switch (normalize(status)) {
                        case "PROCESSING" -> 3;
                        case "READY_FOR_PICKUP", "PACKED", "FAILED" -> 2;
                        default -> 1;
                };
        }
}
