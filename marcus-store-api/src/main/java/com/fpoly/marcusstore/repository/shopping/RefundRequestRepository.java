...
public interface RefundRequestRepository extends JpaRepository<RefundRequest, Long> {
  ...
  @Query("""
      ...
      WHERE r.status = 'PROCESSING'
        AND (r.lastAttemptAt IS NULL OR r.lastAttemptAt <= :before)
      ORDER BY r.lastAttemptAt ASC
      """)
  List<Long> findProcessingIds(@Param("before") LocalDateTime before, Pageable pageable);
}
