...
public interface RefundRequestRepository extends JpaRepository<RefundRequest, Long> {
    ...
    // Marcus thêm danh sách refund đang chờ VNPAY để scheduler đối soát QueryDR.
    @Query("""
            SELECT r.refundId FROM RefundRequest r
            WHERE (r.status = 'PROCESSING'
                AND (r.nextReconciliationAt IS NULL OR r.nextReconciliationAt <= :now))
               OR (r.status = 'SUBMITTING' AND r.lastAttemptAt <= :staleSubmittingBefore)
            ORDER BY r.nextReconciliationAt ASC
            """)
    List<Long> findProcessingIds(
            @Param("now") LocalDateTime now,
            @Param("staleSubmittingBefore") LocalDateTime staleSubmittingBefore,
            Pageable pageable);
}
